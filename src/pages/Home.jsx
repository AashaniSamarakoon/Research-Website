...
function SectionHeader({ label, title, sub }) {
  return (
    <div className="mx-auto max-w-6xl px-6">
      ...
      <h2 className="mt-2 font-serif-display text-3xl tracking-tight text-slate-50 md:text-4xl">{title}</h2>
    ...
    <main>
      ...
      <section id="home" className="relative overflow-hidden">
        ...
        <div className="relative mx-auto flex min-h-screen max-w-6xl flex-col items-center justify-center px-6 pb-16 pt-28 text-center">
          <div className="inline-flex items-center rounded-full border border-secondary/30 bg-secondary/10 px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-secondary">
            Final Year Research Project · 2024/25
          </div>
          <h1 className="mt-6 max-w-4xl font-serif-display text-4xl tracking-tight text-slate-50 sm:text-5xl md:text-6xl">
            {project.name}: <span className="text-secondary">{project.tagline}</span>
          </h1>
          <p className="mt-5 max-w-2xl text-base leading-7 text-slate-300 md:text-lg">
            A complete research showcase including TAF, proposals, reports, methodology, UI designs, interviews, testing, comparison, and the research paper.
          </p>

          <div className="mt-8 flex flex-wrap justify-center gap-2">
            {project.highlights.map((h) => (
              <span key={h} className="rounded-full border border-white/10 bg-white/5 px-4 py-2 text-xs text-slate-300">
                {h}
              </span>
            ))}
          </div>
      ...
      <div className="border-y border-white/10 bg-slate-950/20">
        <div className="mx-auto grid max-w-6xl grid-cols-2 gap-6 px-6 py-10 text-center sm:grid-cols-3 md:grid-cols-5">
          {stats.map((s) => (
            <div key={s.label}>
              <div className="font-serif-display text-3xl text-secondary">{s.value}</div>
              <div className="mt-1 text-xs uppercase tracking-[0.16em] text-slate-400">{s.label}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <section id="about" className="bg-primary py-20">
        ...
        <div className="mx-auto mt-12 grid max-w-6xl grid-cols-1 gap-6 px-6 md:grid-cols-2">
          <div className="rounded-2xl border border-white/10 bg-white/5 p-7">
            <div className="font-serif-display text-2xl text-slate-50">What we’re building</div>
            <p className="mt-3 text-sm leading-7 text-slate-300">
              FreshRoute connects stakeholders in the fruit supply chain using structured batch tracking, AI-assisted decision support, and an integrity-focused event log to reduce disputes and improve operational outcomes.
            </p>
          ...
          <div className="space-y-4">
            {problems.map((p) => (
              <div key={p.title} className="rounded-2xl border border-white/10 bg-white/5 p-6">
                ...
                <p className="mt-2 text-sm leading-7 text-slate-300">{p.body}</p>
              </div>
            ))}
          </div>
          <div className="rounded-2xl border border-white/10 bg-white/5 p-7">
            <div className="text-base font-semibold text-slate-50">Impact snapshot</div>
            <p className="mt-2 text-sm leading-7 text-slate-300">
              We evaluate FreshRoute against baseline workflows using measurable indicators like traceability completeness, task success rate, SUS, and response time.
            </p>
        ...
        <div className="mx-auto mt-12 grid max-w-6xl grid-cols-1 gap-6 px-6 md:grid-cols-2">
          <div className="space-y-4">
            {methodologySteps.map((s, idx) => (
              <div key={s.title} className="flex gap-4 rounded-2xl border border-white/10 bg-white/5 p-6">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm leading-7 text-slate-300">{s.body}</p>
                </div>
              </div>
            ))}
          </div>
          <div className="space-y-6">
            <Card title="Research Philosophy">
              Grounded in pragmatism: we select methods based on what best answers our research questions and fits real operational constraints.
            </Card>
            ...
            <Card title="Ethics & Security">
              Informed consent, anonymization of interview data, access control, and secure storage practices applied throughout the study.
            </Card>
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto mt-12 grid max-w-6xl grid-cols-1 gap-6 px-6 sm:grid-cols-2 lg:grid-cols-4">
          {teamMembers.map((m) => (
            ...
            >
              ...
              <div className="mt-4 text-sm font-semibold text-slate-50">{m.name}</div>
              ...
              <div className="mt-3 inline-flex items-center rounded-full border border-secondary/30 bg-secondary/10 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-secondary">
                {componentsById.get(m.componentId)?.title ?? 'Research Component'}
              </div>
            ...
            <div className="relative w-full max-w-2xl overflow-hidden rounded-2xl border border-white/10 bg-primary shadow-2xl">
              <div className="flex items-start justify-between gap-4 border-b border-white/10 bg-white/5 p-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="text-sm font-semibold text-slate-50">{activeMember.name}</div>
                    <div className="mt-1 text-xs font-semibold uppercase tracking-[0.16em] text-secondary">{activeComponent.title}</div>
                  </div>
                </div>
                <button
                  type="button"
              ...
              <div className="p-6">
                <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                  <div>
                    <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Member overview</div>
                    <p className="mt-2 text-sm leading-7 text-slate-300">{activeMember.bio}</p>

                    <div className="mt-5 text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Component tags</div>
                  ...
                  <div>
                    <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Component summary</div>
                    <p className="mt-2 text-sm leading-7 text-slate-300">{activeComponent.body}</p>

                    <div className="mt-4">
                      <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Key points</div>
                ...
                <div className="mt-6">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Related documents</div>
                  <div className="mt-3 space-y-2">
                    {activeComponent.relatedDocIds.map((docId) => {
                      ...
                      return (
                        ...
                        >
                          <div className="text-sm font-semibold text-slate-50">{doc.title}</div>
                          <div className="mt-0.5 text-xs text-slate-400">{doc.meta}</div>
                        </a>
                      )
                    })}
                  </div>
            ...
            {filteredDocs.map((d) => (
              ...
              >
                <div className="flex-1">
                  <div className="text-sm font-semibold text-slate-50">{d.title}</div>
      ...
      <section id="ui-designs" className="bg-primary py-20">
        ...
        <div className="mx-auto mt-12 grid max-w-6xl grid-cols-1 gap-6 px-6 md:grid-cols-3">
          {uiDesigns.map((u) => (
            <div key={u.title} className="rounded-2xl border border-white/10 bg-white/5 p-6">
              <div className="text-sm font-semibold text-slate-50">{u.title}</div>
              <div className="mt-2 rounded-xl border border-white/10 bg-slate-950/30 p-4">
                <div className="h-2 w-2 rounded-full bg-red-400/80" />
                <div className="mt-4 space-y-2">
                  <div className="h-3 w-2/3 rounded bg-white/10" />
                  <div className="h-3 w-full rounded bg-white/10" />
                  <div className="h-3 w-5/6 rounded bg-white/10" />
                  <div className="mt-6 h-9 w-28 rounded bg-secondary/30" />
                </div>
              </div>
        ...
        <div className="mx-auto mt-10 flex max-w-6xl flex-col items-start justify-between gap-4 rounded-2xl border border-white/10 bg-white/5 px-6 py-6 sm:flex-row sm:items-center">
          <div>
            <div className="text-sm font-semibold text-slate-50">Full design files available</div>
            <p className="mt-1 text-sm text-slate-300">Figma prototypes, user flows, and accessibility notes</p>
          </div>
          <a
            className="inline-flex items-center rounded-lg bg-secondary px-5 py-3 text-sm font-semibold text-slate-950 hover:opacity-90"
            href="#"
      ...
      <section id="interviews" className="bg-slate-950/20 py-20">
        ...
        <div className="mx-auto mt-12 grid max-w-6xl grid-cols-1 gap-6 px-6 md:grid-cols-2">
          {interviews.map((i) => (
            <div key={i.who} className="rounded-2xl border border-white/10 bg-white/5 p-6">
              <div className="font-serif-display text-lg italic text-slate-50">“{i.quote}”</div>
              <div className="mt-3 text-xs uppercase tracking-[0.16em] text-slate-400">{i.who}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="testing" className="bg-primary py-20">
        ...
        <div className="mx-auto mt-12 grid max-w-6xl grid-cols-2 gap-4 px-6 sm:grid-cols-4">
          {testing.metrics.map((m) => (
            <div key={m.label} className="rounded-2xl border border-white/10 bg-white/5 p-5 text-center">
              <div className="font-serif-display text-3xl text-secondary">{m.value}</div>
              <div className="mt-1 text-xs uppercase tracking-[0.16em] text-slate-400">{m.label}</div>
            </div>
          ))}
        </div>
              ...
              <tbody className="divide-y divide-white/5">
                {comparisonRows.map((r) => (
                  <tr key={r.criteria} className="hover:bg-white/5">
                    <td className="px-5 py-4 font-semibold text-slate-100">{r.criteria}</td>
                    ...
                    <td className="px-5 py-4 font-semibold text-secondary">{r.ours}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section id="paper" className="bg-primary py-20">
        ...
        <div className="mx-auto mt-12 max-w-6xl px-6">
          <div className="rounded-2xl border border-secondary/30 bg-secondary/5 p-7">
            ...
            <h3 className="mt-4 font-serif-display text-2xl tracking-tight text-slate-50 md:text-3xl">{paper.title}</h3>
        ...
        <div className="mx-auto mt-12 grid max-w-6xl grid-cols-1 gap-6 px-6 lg:grid-cols-2">
          <div className="overflow-hidden rounded-2xl border border-white/10 bg-white/5">
            <div className="border-b border-white/10 bg-white/5 px-6 py-4">
              <div className="text-sm font-semibold text-slate-50">Business Poster</div>
              <div className="mt-1 text-xs uppercase tracking-[0.16em] text-slate-400">Replace with your official poster if needed</div>
            </div>
            <div className="p-6">
              <img
          ...
          <div className="space-y-6">
            <Card title="Business Accounts">
              <div className="mt-3 space-y-3">
                {commercialization.businessAccounts.map((a) => (
                  ...
                  >
                    <div className="text-sm font-semibold text-slate-50">{a.label}</div>
                    <div className="mt-1 text-sm text-slate-300">{a.handle}</div>
                  </a>
                ))}
              </div>
            </Card>
            ...
            <Card title={commercialization.pro.title}>
              <p className="mt-2 text-sm leading-7 text-slate-300">{commercialization.pro.body}</p>
      ...
      <section id="contact" className="bg-primary py-20">
        <SectionHeader
          label="Get in Touch"
          title="Contact"
          sub="Use the channels below for research collaboration, demos, or partnership discussions."
        />

        <div className="mx-auto mt-12 grid max-w-6xl grid-cols-1 gap-6 px-6 md:grid-cols-2">
          <Card title="General Email">
            <a className="text-slate-100 underline decoration-white/20 underline-offset-4 hover:decoration-secondary" href={`mailto:${contact.generalEmail}`}>
              {contact.generalEmail}
            </a>
          </Card>

          <Card title={contact.hotline.label}>
            <a className="text-slate-100 underline decoration-white/20 underline-offset-4 hover:decoration-secondary" href={`tel:${contact.hotline.phone}`}>
              {contact.hotline.phone}
            </a>
          </Card>

          <Card title={contact.phone.label}>
            <a className="text-slate-100 underline decoration-white/20 underline-offset-4 hover:decoration-secondary" href={`tel:${contact.phone.phone}`}>
              {contact.phone.phone}
            </a>
          </Card>
